        ...
        {sorted.map(sym=>(
          <div key={sym.sym} onClick={()=>setSelected(sym.sym===selected?null:sym.sym)} style={{
            ...
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8 }}>
              <div>
                <div style={{ fontWeight:700,fontSize:14 }}>{sym.sym.replace('','')}<span style={{ color:T.muted,fontSize:10 }}>/</span></div>
      ...
      {sel && (
        <div style={{ display:'flex',flexDirection:'column',gap:12,marginTop:4 }}>
          ...
          <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 18px',background:T.card,borderRadius:10,border:`1px solid ${T.accentGlow}` }}>
            <div>
              <div style={{ fontSize:20,fontWeight:700,letterSpacing:-0.3 }}>{sel.sym.replace('','')}<span style={{ color:T.muted,fontSize:12 }}>/</span></div>
