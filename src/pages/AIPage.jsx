...
const QUICK_PROMPTS = [
  'What are my biggest weaknesses as an F&O trader?',
  'Which hours and days am I most profitable?',
  'Am I over-trading options? Assess my position sizing.',
  'Which instruments should I focus on — NIFTY, BANKNIFTY or equity?',
  'Analyze my losing streak patterns',
  'How do my CE vs PE trades compare? Any directional bias?',
  ...
  'How should I adjust my strategy for the current market?',
]

async function callGrok(trades, stats, userPrompt) {
  const summary = {
    ...
    topInstruments: (stats.symbolArr||[]).slice(0,5).map(s=>({ symbol:s.sym, pnl:+fmt(s.pnl), winRate:s.wr, trades:s.count })),
    worstInstruments: (stats.symbolArr||[]).slice(-3).map(s=>({ symbol:s.sym, pnl:+fmt(s.pnl), winRate:s.wr })),
    ...
    monthlyTrend:  (stats.monthlyArr||[]).map(m=>({ month:m.label, pnl:m.pnl, wr:m.wr, fees:m.fees })),
    last10Trades:  trades.slice(-10).map(t=>({ sym:t.symbol, side:t.side, segment:t.segment, pnl:+t.pnl.toFixed(2) })),
  }

  const system = `You are an elite prop trading coach specialising in Indian equity and F&O markets (NSE/BSE/NFO) with 15+ years experience. You understand options Greeks, Nifty/BankNifty expiry dynamics, SEBI regulations, intraday MIS vs NRML, STT implications, and Indian market microstructure.

Analyse the trading data with clinical precision and brutal honesty. Be direct — no fluff, no generic advice.
...
## Key Strengths  
## Critical Weaknesses
## Segment Analysis (Equity / Options / Futures)
## Timing & Pattern Insights
## Instrument Analysis
## Top 5 Immediate Actions
## 30-Day Performance Target

Reference specific numbers from the data. Give an overall score /10 in the verdict. Keep each section sharp and actionable.`

  const userMsg = userPrompt
    ? `${userPrompt}\n\nMy trading stats (INR):\n${JSON.stringify(summary, null, 2)}`
...
export default function AIPage({ trades, stats }) {
  ...
  const run = async (p) => {
    ...
    try {
      const result = await callGrok(trades, stats, q)
      setAnalysis(result)
    ...
    <div style={{ padding:'24px 28px', fontFamily:T.fontSans }}>
      <div style={{ marginBottom:24, paddingBottom:16, borderBottom:`1px solid ${T.border}` }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:4 }}>
          ...
          <span style={{ fontSize:12, color:T.muted }}>Powered by <span style={{ color:'#00e5ff', fontWeight:600 }}>Grok</span> (xAI) · {(trades||[]).length} trades analysed</span>
        </div>
      </div>

      {/* Stats snapshot */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:10, marginBottom:20 }}>
        <KpiCard label="Net P&L"    value={((stats.netPnL||0)>=0?'+₹':'-₹')+fmt(Math.abs(stats.netPnL||0))} color={colorPnL(stats.netPnL||0)}/>
        <KpiCard label="Win Rate"   value={fmt(stats.winRate||0)+'%'}  color={(stats.winRate||0)>=50?T.green:T.red}/>
        <KpiCard label="R:R"        value={(stats.rr||0)+'x'}          color={parseFloat(stats.rr||0)>=1.5?T.green:T.red}/>
        <KpiCard label="Total Fees" value={'₹'+fmt(stats.totalFees||0)} color={T.red}/>
        <KpiCard label="Drawdown"   value={fmt(stats.maxDD||0)+'%'}    color={(stats.maxDD||0)<15?T.green:T.red}/>
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 260px', gap:16, alignItems:'start' }}>
        <div>
          <Card style={{ marginBottom:12 }} glow>
            <SectionHead title="Ask Grok" sub="Custom Query"/>
            <textarea value={prompt} onChange={e=>setPrompt(e.target.value)}
              placeholder="Ask something specific... e.g. 'Why am I losing on BANKNIFTY options?' or leave blank for full analysis"
              rows={4} style={{
                width:'100%', background:T.surface, border:`1px solid ${T.border}`, borderRadius:8,
                padding:'12px', color:T.text, fontFamily:T.fontSans, fontSize:12,
            ...
            <div style={{ display:'flex', gap:10 }}>
              <Btn variant="accent" onClick={()=>run()} disabled={loading} style={{ flex:1, padding:'10px', fontSize:13 }}>
                {loading ? 'Analysing...' : '⚡ Run Grok Analysis'}
              </Btn>
            ...
            {error && (
              <div style={{ marginTop:12, background:T.redDim, border:`1px solid ${T.red}55`, borderRadius:8, padding:'14px 16px', lineHeight:1.8 }}>
                <div style={{ color:T.red, fontWeight:600, fontSize:12, marginBottom:6 }}>⚠ Error</div>
                <div style={{ color:T.red, fontSize:12, whiteSpace:'pre-wrap' }}>{error}</div>
                <div style={{ marginTop:10, paddingTop:10, borderTop:`1px solid ${T.red}33`, fontSize:11, color:T.muted }}>
                  Add <code style={{ color:T.accent, background:T.surface, padding:'2px 6px', borderRadius:3 }}>GROK_API_KEY</code> to Netlify environment variables.
                </div>
              </div>
            )}
          </Card>
          ...
          {loading && (
            <Card style={{ textAlign:'center', padding:'48px 24px' }}>
              <div style={{ display:'flex', justifyContent:'center', marginBottom:16 }}><Spinner size={36} color="#00e5ff"/></div>
              <div style={{ fontWeight:600, fontSize:15, marginBottom:8 }}>Grok is analysing {(trades||[]).length} trades...</div>
              <div style={{ color:T.muted, fontSize:12 }}>Reviewing your performance, patterns and risk</div>
            </Card>
          )}
          ...
          {analysis && !loading && (
            <Card glow>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                <SectionHead title="Analysis Result" sub="Grok · xAI"/>
          ...
          {!analysis && !loading && (
            <div style={{ textAlign:'center', padding:'60px 20px', background:T.card, border:`1px solid ${T.border}`, borderRadius:10 }}>
              ...
              <div style={{ color:T.muted, fontSize:12, marginBottom:24, lineHeight:1.7, maxWidth:360, margin:'0 auto 24px' }}>
                Get a comprehensive Indian F&O trading review covering strengths, weaknesses, segment breakdown, timing patterns, and a personalised improvement plan.
              </div>
              <Btn variant="accent" onClick={()=>run('')} style={{ padding:'11px 32px', fontSize:13 }}>Run Full Grok Analysis</Btn>
            </div>
          )}
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          {history.length > 0 && (
            <Card>
              <SectionHead title="History" sub="Recent"/>
              <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {history.map((h,i)=>(
          ...
          <Card>
            <SectionHead title="Setup" sub="Grok API"/>
