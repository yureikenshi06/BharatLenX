...
const QUICK_PROMPTS = [
  ...
  'Which F&O instruments should I focus on (NIFTY/BANKNIFTY options vs equity)??',
  'Analyze my losing streak patterns',
  'How do my CE vs PE trades compare? Any bias?',
...
async function callAI(trades, stats, userPrompt) {
  ...
  const system = `You are an elite prop trading coach specialising in Indian Indian F&O with 15+ years experience.
...
Reference specific numbers from the data. Give an overall score /10 in the verdict. Keep each section concise and actionable.`
  ...
  const msg = userPrompt
    ...
    : `Full analysis of my Indian F&O trading.\n\nStats:\n${JSON.stringify(summary,null,2)}`
  ...
  return (
    <div style={{ padding:'24px 28px',fontFamily:T.fontSans }}>
      <div style={{ marginBottom:24,paddingBottom:16,borderBottom:`1px solid ${T.border}` }}>
        ...
        <div style={{ fontSize:12,color:T.muted,marginTop:4 }}>Powered by Claude Sonnet (Free) · {trades.length} trades analysed</div>
      </div>
        ...
        <div>
          <Card style={{ marginBottom:12 }} glow>
            ...
            <textarea value={prompt} onChange={e=>setPrompt(e.target.value)}
              placeholder="Ask something specific... e.g. 'Why am I losing on ETH?' or leave blank for full analysis"
              rows={4} style={{
