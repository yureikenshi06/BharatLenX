...
function DayGoalsTracker({ dayTrades, dayPnL, dayFees }) {
  const [goals, setGoals] = useState(() => {
    try { return JSON.parse(localStorage.getItem('ml_day_goals') || '{}') } catch { return {} }
  })
  const [editing, setEditing] = useState(false)
  const [form, setForm] = useState({ ...DEFAULT_DAY_GOALS, ...goals })
  const g = { ...DEFAULT_DAY_GOALS, ...goals }

  const save = () => { localStorage.setItem('ml_day_goals', JSON.stringify(form)); setGoals(form); setEditing(false) }
...
function useEOD() {
  const [entries, setEntries] = useState(() => { try { return JSON.parse(localStorage.getItem('ml_eod')||'[]') } catch { return [] } })
  const save = (entry) => {
    const updated = [entry, ...entries.filter(e=>e.date!==entry.date)]
    localStorage.setItem('ml_eod', JSON.stringify(updated))
    setEntries(updated)
  }
  return { entries, save }
}
...
export default function EODPage({ trades, stats }) {
  ...
  const [form, setForm] = useState(() => {
    try { return JSON.parse(localStorage.getItem('ml_eod')||'[]').find(e=>e.date===today) || { mood:'',lessons:[],notes:'',rating:0,followedPlan:null } }
                  ...
                  <div key={t.id} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 12px',background:T.surface,borderRadius:7,border:`1px solid ${colorPnL(t.pnl)}25` }}>
                    <div>
                      <div style={{ display:'flex',gap:8,alignItems:'center',marginBottom:3 }}>
                        <span style={{ fontWeight:600,fontSize:13 }}>{t.symbol.replace('','')}/</span>
