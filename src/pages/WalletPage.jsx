...
function useLiveData() {
  ...
  const refresh = useCallback(async () => {
    const keys = loadKeys()
    if (!keys?.clientId || !keys?.token) { setError('No Fyers keys saved. Connect in Settings first.'); return }
    setLoading(true); setError('')
    try {
      const data = await fetchLiveAccount(keys.clientId, keys.token)
    ...
    <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
      ...
      <Card glow>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10 }}>
          ...
          <div>
            <div style={{ fontSize:10,color:T.muted,letterSpacing:1,textTransform:'uppercase',marginBottom:5 }}>Amount ()</div>
  ...
  return (
    <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10 }}>
        <KpiCard label="Capital Balance"      value={'₹'+fmt(account.totalCapitalBalance)}   color={T.accent}  sub="Total "/>
              ...
              <tbody>
                {account.positions.map((p,i)=>(
                  <tr key={p.symbol} style={{ borderBottom:`1px solid ${T.border}`,background:i%2===0?'transparent':T.surface+'66' }}>
                    <td style={{ padding:'10px 12px',fontWeight:700 }}>{p.symbol.replace('','')}<span style={{ color:T.muted,fontSize:10 }}>/</span></td>
  ...
  return (
    <div style={{ padding:'24px 28px',fontFamily:T.fontSans }}>
      <div style={{ marginBottom:20,paddingBottom:14,borderBottom:`1px solid ${T.border}` }}>
        ...
        <div style={{ fontSize:22,fontWeight:700,letterSpacing:-0.5 }}>Capital & Capital</div>
      </div>
